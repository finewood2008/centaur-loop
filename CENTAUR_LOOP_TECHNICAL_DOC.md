...
interface CentaurLoopConfig {
  ...
  hasFastLoop?: boolean;          // 是否有快闭环（日级）
  hasSlowLoop?: boolean;          // 是否有慢闭环（周级）
}
```
...
#### 短视频生产闭环（videoLoop.ts）

- 周期：每天一轮
- 触发：AI 每天早上推荐今日选题
- AI 工具：短视频脚本生成
- 人工卡点：确认选题（30min 提醒）→ 确认脚本（1h 提醒）→ 拍摄并发布（8h 提醒）→ 回传数据（4h 提醒，必须）→ 确认经验（30min 提醒，可跳过）
- 记忆类别：有效钩子、最佳时长、热门话题、发布时间、平台偏好、评论洞察
- 特殊：支持快慢双闭环（`hasFastLoop` + `hasSlowLoop`）
...
短视频闭环引入了「快闭环 + 慢闭环」的概念：

- **快闭环（Fast Loop）**：每天一轮，粒度是单条视频。选题→脚本→拍摄→发布→数据回收→经验沉淀。节奏快，关注日级执行。
- **慢闭环（Slow Loop）**：每周一轮，粒度是整周策略。汇总本周所有快闭环数据→分析趋势→调整方向→下周计划。节奏慢，关注周级进化。

快闭环的记忆沉淀喂给慢闭环的复盘，慢闭环的策略建议反过来指导快闭环的选题——形成**双轮驱动的螺旋进化**。

当前实现中通过 `hasFastLoop` 和 `hasSlowLoop` 标志位标识，具体的双闭环编排逻辑待后续迭代。
...
7. **跨闭环记忆共享**：不同闭环的经验可选择性共享（如 SEO 闭环的选题经验可喂给视频闭环）。
